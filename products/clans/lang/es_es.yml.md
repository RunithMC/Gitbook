...
messages:
  ...
  in-process: "&8» &cEspera a que se termine la tarea de: %action% para el clan %clan%"

  format:
    - "&3&lRunithClans"
    - " "
    - "&fComandos disponibles &7→"
    - " &8● &bcreate &9(nombre) (tag) &8- &fCrea un clan"
    - " &8● &bdisband &8- &fElimina tu clan"
    - " &8● &bleave &8- &fAbandona tu clan"
    - " &8● &bkick &9(jugador) &8- &fExpulsa a un jugador"
    - " &8● &btag &9(nuevo) &8- &fCambia el tag del clan"
    - " &8● &blist &9(página) &8- &fMuestra los clanes conectados"
    - " &8● &binfo &9(clan) &8- &fMuestra la información de un clan"
    - " &8● &bchat &9(enemy) (clan enemigo) &8- &fAlterna entre chats"
    - " &8● &binvite &9(accept/deny/jugador) &8- &fGestiona invitaciones"
    - " &8● &bsetrole &9(jugador) (rol) &8- &fCambia el rol de un miembro"
    - " &8● &bbank &9(withdraw/deposit) (cantidad) &8- &fAdministra el banco del clan"
    - " &8● &bally &9(add/remove/list) (nombre) &8- &fGestiona alianzas"
    - " &8● &benemy &9(add/remove) (nombre) &8- &fGestiona enemigos"
    - " &8● &bupgrades &8- &fCompra mejoras para el clan"
    - " &8● &bbase &9(set) &8- &fIr/cambiar la base del clan"
    - " &8● &brally &8- &fConvoca a todos tus miembros"
    - " &8● &bfriendlyfire &8- &fActiva o desactiva el PVP entre miembros"
    - " &8● &brename &9(nombre) (tag) &8- &fCambia el nombre al clan"
  ...
  create:
    ...
    name_to_large: "&8» &cEl nombre del clan no puede superar los &e16 &ccaracteres."
  ...
  leave:
    is-leader: "&8» &cNo puedes abandonar el clan siendo líder. Usa &b/clan disband &co transfiere el liderazgo."
  ...
  info:
    ...
    footer:
      - " "
      - "&8Estadísticas:"
      - "&8» &fPuntos: &b%points% &8| &fKills: &b%kills% &8| &fMuertes: &b%deaths%"
      - "&8» &fDinero: &b%money%"
  ...
  friendlyfire:
    ...
    no-in-clan: "&8» &cNo perteneces a ningún clan."
    need-higher-role: "&8» &cDebes ser al menos &eCO-LÍDER &cpara cambiar esta configuración."

  chat:
    format:
      - "&8» &cUso: &f/clan chat &b(enemy) (clan enemigo)"
      - "&8» &cUso: &f/clan chat &bally"
      - "&7Usa solo &f/clan chat &7para alternar entre chat global y de clan."
    no-found-enemy:
      - "&8» &cNo se encontró el clan enemigo &b%enemy%&c."
      - "&7Enemigos actuales: &b%enemies%"
    no-online-enemy: "&8» &cNingún miembro del clan enemigo está conectado."
    enemy-offline: "&8» &cEl clan enemigo %enemy% no está conectado."
    global: "&8» &fHas cambiado al chat &bGLOBAL&f."
    clan: "&8» &fHas cambiado al chat del &bCLAN&f."
    enemy: "&8» &fAhora puedes hablar con el clan enemigo &b%enemy_tag% &8(&7%enemy%&8)&f."
    no-in-clan: "&8» &cNo perteneces a ningún clan."
    no-allies: "&8» &cTu clan no tiene aliados."
    no-online-allies: "&8» &cNo hay aliados conectados."
    ally-offline: "&8» &cEl clan aliado %ally% no está conectado."
    ally: "&8» &fHas cambiado al chat de &a&lALIADOS&f."
  ...
  invite:
    ...
    MAX_MEMBERS: "&8» &cTu clan alcanzó el límite de &e%max% &cmiembros."
    ...
    info-to-members:
      ...
      DENIED: "&8» &eEl jugador &b%target% &cha rechazado &eunirse al clan."
      INVITED: "&8» &fEl líder &b%sender% &fha invitado a &b%target%&f."
      ACCEPTED: "&8» &fEl jugador &b%target% &fha &aaceptado &funirse al clan."

  kick:
    format:
      - "&8» &cUso: &f/clan kick &b(jugador) (motivo)"
      - "&7Ejemplo: /clan kick &bPepito &fComportamiento inadecuado"
    no-in-clan: "&8» &cNo perteneces a ningún clan."
    ...
    info-to-kicked: "&8» &cHas sido expulsado del clan por &b%player% &c(motivo: &e%reason%&c)."
  ...
  set-role:
    ...
    high-rank: "&8» &cNo puedes modificar el rol de un jugador con igual o mayor rango."
    need-high-rank: "&8» &cNo tienes el rango necesario para realizar esta acción."
    min-rank: "&8» &cSolo los &eCO-LÍDERES &cpueden cambiar roles."
    ...
    leader-confirm:
      ...
      - "&ePara darle el lider a otra persona, automaticamente después de eso"
      - "&ete volverás un co-lider."
  upgrade:
    no-in-clan: "&8» &cNo perteneces a ningún clan."
    max-level: "&8» &cLa mejora &b%upgrade% &cya alcanzó su nivel máximo."
    no-enough-balance: "&8» &cTu clan no tiene fondos suficientes. Requiere &e%amount%&c, disponible: &e%clan_balance%&c."
    info-upgrade: "&8» &fEl jugador &b%player% &fmejoró &b%upgrade% &fa nivel &b%level%&f."
  ...
  bank:
    ...
    error:
      ...
      FOUND_SIGN_CHARACTER: "&8» &cNo incluyas signos de + o - en la cantidad."
      ...
      clan-no-has-enough-money: "&8» &cEl clan no tiene suficientes fondos (&b%clan_amount%&c)."
  ...
  enemy:
    ...
    no-in-clan: "&8» &cNo perteneces a ningún clan."
    need-high-rank: "&8» &cDebes ser al menos &eCO-LÍDER &cpara modificar enemigos."
    max-enemies: "&8» &cTu clan alcanzó el límite de &e%max% &cenemigos."
    no-online: "&8» &cSolo puedes declarar enemigo a un clan conectado."
    add: "&8» &fEl jugador &b%player% &fha declarado enemigo al clan &b%enemy_tag% &8(&7%enemy_name%&8)&f."
  ...
  ally:
    ...
    no-in-clan: "&8» &cNo perteneces a ningún clan."
    min-rank: "&8» &cNecesitas ser al menos Co-Líder para gestionar alianzas."
    clan-not-found: "&8» &cEl clan &b%clan% &cno fue encontrado."
    cannot-ally-yourself: "&8» &cNo puedes hacer alianza con tu propio clan."
    already-allied: "&8» &cYa tienes alianza con &b%clan%&c."
    cannot-ally-enemy: "&8» &cNo puedes tener alianza con &b%clan% &cporque es tu enemigo."
    max-allies: "&8» &cNo puedes tener más de &b%max% &calianzas."
    target-max-allies: "&8» &cEl clan &b%clan% &cya tiene el máximo de alianzas."
    not-allied: "&8» &cNo tienes alianza con &b%clan%&c."
    no-pending-request: "&8» &cNo hay solicitud de alianza pendiente con &b%clan%&c."
    no-permission: "&8» &cNo tienes permisos para gestionar alianzas."

    request-sent: "&8» &aSolicitud de alianza enviada a &b%clan%&a."
    request-received: "&8» &b%clan% &asolicita una alianza contigo &8(&7enviado por &b%sender%&8)"
    ...
    cannot-damage: "&8» &cNo puedes atacar a miembros de &b%clan% &cporque es aliado."

  rally:
    no-in-clan: "&8» &cNo perteneces a ningún clan."
    info-to-members: "&3[RALLY] &fEl jugador &a%player_name% &fsolicita apoyo en &a%worldguard_region_name%&f."
    expired: "&4⚠ &cEl rally fue eliminado por que expiró."

  base:
    no-in-clan: "&8» &cNo perteneces a ningún clan."
    info-to-members: "&3[BASE] &fEl jugador &a%player_name% &fha cambiado la base a &e%x%, %y%, %z%."
    need-higher-role: "&8» &cDebes ser al menos &eLÍDER &cpara cambiar la base."
    in-blacklisted-world: "&8» &cEstás en un mundo prohibido, no puedes ir/cambiar la base."
    no-exists: "&8» &cTu clan no tiene ninguna base establecida."
  ...
  rename:
    ...
    info-to-members: "&3[RENAME] &fEl jugador &a%player_name% &fha cambiado el nombre del clan a %clan% con el tag de %tag%&f."
    need-higher-role: "&8» &cDebes ser al menos &eCO_LÍDER &cpara cambiar el nombre."
    format: "&8» &cPara renamear tu clan, usa: /clan rename (nombre) (tag)."

    already_exist: "&8» &cEl clan &b%clan% &cya existe."
    already_in_clan: "&8» &cYa estás en un clan. Usa &b/clan leave &cpara salir antes de crear otro."
    name_to_large: "&8» &cEl nombre del clan no puede superar los &e16 &ccaracteres."
    renamed: "&8» &fHas cambiado el nombre a &b%clan% &fcon el tag &b%tag%&f."
    invalid_name: "&8» &cEl nombre ingresado no es válido. Solo se permiten letras, números y algunos símbolos."
    cooldown: "&8» &cNecesitas esperar &e%time%&c antes de cambiar el nombre del clan de vuelta"
```
{% endcode %}
